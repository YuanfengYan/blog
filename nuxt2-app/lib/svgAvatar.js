 ...
 * 
 * @param {*} string string
 * @param {*} sansEnv boolean
 * @param {*} ver Object
 * @returns 
 */
export default function svgCreate(string, sansEnv, ver) {
  // string += '';
  // 部件颜色
  const themes = {
    // Robo
  ...
  const svgEnd = '</svg>';
  const env = '<path d="M33.83,33.83a115.5,115.5,0,1,1,0,163.34,115.49,115.49,0,0,1,0-163.34Z" style="fill:#01;"/>';
  const head = '<path d="m115.5 51.75a63.75 63.75 0 0 0-10.5 126.63v14.09a115.5 115.5 0 0 0-53.729 19.027 115.5 115.5 0 0 0 128.46 0 115.5 115.5 0 0 0-53.729-19.029v-14.084a63.75 63.75 0 0 0 53.25-62.881 63.75 63.75 0 0 0-63.65-63.75 63.75 63.75 0 0 0-0.09961 0z" style="fill:#000;"/>';
  ...
  const sha256Hash = CryptoJS.SHA256(string);
  ...
  for (const part in p) {
    let partV = p[part].substring(0,2);
    let theme = p[part].substring(2,3);
