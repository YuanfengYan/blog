<template>
  <div class="form-list">
    <div class="logo">
      <img src="http://rkrvzzlak.hd-bkt.clouddn.com/logo.png" alt="logo" />
    </div>
    ...
    <button class="anonymous-comment" @click="onAnonymousComment">匿名评论</button>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async onUserRegister() {
      ...
      const [err] = await this.$store.dispatch("user/userRegister", registerParams);
      if (!err) {
        this.onSuccess();
