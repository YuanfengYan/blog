<template>
  <div>
    ...
    <div v-if="count > 0" class="comment-container">
        ...
        <li v-for="(item, index) in commentList" :key="item.id" class="comment-item">
          ...
          <div class="comment-item-detail">
            ...
            <div v-if="item.reply_list" class="comment-item-reply-content">
              ...
              >
                {{
                  (replyItem.user_info && replyItem.user_info.username) || "匿名回复"
                }}：
                {{ replyItem.content }}
              </p>
            </div>
            <div v-if="item.is_show_reply" class="comment-item-reply">
              <textarea
                ...
                placeholder="发表回复…"
              />
              <button class="comment-item-reply-btn" @click="submitReply(item, index)">
                回复
              </button>
            </div>
          </div>
      ...
      <div v-if="isLoad" class="comment-more" @click="loadMore">点击加载更多</div>
    </div>
